...
const Header: FC<HeaderProps> = ({ activeTab, hasNotifications }) => {
  ...
  useEffect(() => {
    ...
    const UpdateTimeDate = () => {
      const _Now = new Date()
      setTimeString(_Now.toLocaleTimeString('pl-PL', { hour: '2-digit', minute: '2-digit' }))
      setDateString(_Now.toLocaleDateString('pl-PL', { weekday: 'long', day: 'numeric', month: 'long' }))
    }
  ...
  useEffect(() => {
    ...
    if (SpotifyTrack.isPlaying && SpotifyTrack.progress !== undefined && SpotifyTrack.duration !== undefined) {
      _ProgressInterval.current = setInterval(() => {
        const ElapsedTime = Date.now() - _LastUpdateTime.current
        const NewProgress = Math.min(SpotifyTrack.duration!, (SpotifyTrack.progress || 0) + ElapsedTime)
  ...
  return (
    <>
                  ...
                  >
                    <div className="text-xs text-rose-200/70 font-medium tracking-wide flex items-center mt-1.5">
                      ...
                      <span className="inline-flex items-center gap-2">
                        <span className="backdrop-blur-sm bg-rose-500/5 px-2 py-0.5 rounded-md border border-rose-300/10">{TimeString}</span>
                        <span className="text-rose-300/50 flex items-center justify-center">•</span>
                        <span className="backdrop-blur-sm bg-rose-500/5 px-2 py-0.5 rounded-md border border-rose-300/10">{DateString}</span>
                      </span>
                    </div>
                  </div>
                </div>
              ...
              {!IsScrolled && (
                <div className="flex items-center space-x-8">
                  <div className="flex items-center bg-rose-500/10 hover:bg-rose-500/15 rounded-xl px-5 py-2.5 border border-rose-500/20 hover:border-rose-500/30 transition-all group select-none">
                    ...
                    <div className="flex flex-col">
                      <span className="text-white/90 text-sm font-medium group-hover:text-white">{WeatherData.temp}</span>
                      <span className="text-rose-200/50 text-xs group-hover:text-rose-200/70">{WeatherData.condition}</span>
                    </div>
                  </div>
                  ...
                  <div className="relative flex items-center bg-rose-500/10 hover:bg-rose-500/15 rounded-xl border border-rose-500/20 hover:border-rose-500/30 transition-all group select-none overflow-hidden">
                    ...
                    <div className="flex items-center px-5 py-2.5">
                      <div className="w-8 h-8 rounded-full bg-green-500/20 flex items-center justify-center mr-3">
                        <i className={`fab fa-spotify text-green-500 ${SpotifyTrack.isPlaying ? 'animate-pulse' : ''}`}></i>
                      </div>
                      <div className="flex flex-col">
                        <span className="text-white/90 text-sm font-medium truncate max-w-[140px] group-hover:text-white">{SpotifyTrack.title}</span>
      ...
      <style jsx global>{`
        .bg-grid-pattern {
          background-image: 
            linear-gradient(to right, rgba(244, 63, 94, 0.1) 1px, transparent 1px),
            linear-gradient(to bottom, rgba(244, 63, 94, 0.1) 1px, transparent 1px);
