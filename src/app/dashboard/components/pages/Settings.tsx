...
import { SetLessonDuration, GetCurrentLessonDuration, InitializeLessonDuration } from '@/lib/data/LessonTimes/LessonTimesUtil'
...
const Settings: FC<SettingsProps> = ({ username, defaultLessonTime = 45 }) => {
  ...
  const CanAuthorizeSpotify = HasPermission(UserPermissions, Permission.SPOTIFY_AUTH)
  const CanEditClassTimes = HasPermission(UserPermissions, Permission.CLASS_TIMES_EDIT)
  const CanViewClassTimes = HasPermission(UserPermissions, Permission.CLASS_TIMES_VIEW)
  
  const CanEditSpotifySettings = CanAuthorizeSpotify || HasPermission(UserPermissions, Permission.ADMINISTRATOR)
  const CanEditLessonTimes = CanEditClassTimes || HasPermission(UserPermissions, Permission.ADMINISTRATOR)
              ...
              <div className="flex items-center">
                <div 
                  ...
                  className={`relative flex bg-white/5 border border-rose-500/20 rounded-lg overflow-hidden w-24 cursor-pointer select-none transition-opacity duration-300 ${
                    ...
                    !CanEditLessonTimes ? 'opacity-50 cursor-not-allowed' : 'opacity-100'
                  }`}
