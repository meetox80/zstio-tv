import { FC, useState, useRef, DragEvent, useEffect, KeyboardEvent, ChangeEvent } from 'react'
...
const Slajdy: FC<SlajdyProps> = () => {
  ...
  const HandleDragOverSlide = (Id: string, Event: React.DragEvent<HTMLDivElement>) => {
  ...
  const HandleDropOnSlide = async () => {
    ...
    const DraggedIndex = _Slides.findIndex(slide => slide.Id === DraggedSlide)
    ...
    NewSlides.splice(FinalDropIndex > DraggedIndex ? FinalDropIndex - 1 : FinalDropIndex, 0, DraggedItem)
  ...
  const HandleKeyDown = (Event: KeyboardEvent<HTMLDivElement>, Id: string) => {
    if ((Event.key === 'Delete' || Event.key === 'Backspace') && !DeletingSlideId) {
      Event.preventDefault()
  ...
  const HandleContainerKeyDown = (Event: KeyboardEvent<HTMLDivElement>) => {
    if ((Event.key === 'Delete' || Event.key === 'Backspace') && SelectedSlide && !DeletingSlideId) {
      Event.preventDefault()
  ...
  useEffect(() => {
    const HandleGlobalKeyDown = (Event: globalThis.KeyboardEvent) => {
      if ((Event.key === 'Delete' || Event.key === 'Backspace') && SelectedSlide && !DeletingSlideId && document.activeElement === SlideContainerRef.current) {
        Event.preventDefault()
        RequestRemoveSlide(SelectedSlide.Id)
      }
    }
              ...
              {SelectedSlide ? (
                ...
                >
                  ...
                  <div className="absolute bottom-6 left-1/2 -translate-x-1/2 flex items-center gap-3 px-4 py-2 bg-black/60 backdrop-blur-md rounded-full border border-gray-700/50 z-20">
                    <button 
                      onClick={() => HandleDownload(SelectedSlide.ImageData, SelectedSlide.Name)}
                    ...
                    <div className="flex items-center">
                      ...
                      <span className="text-white text-sm font-medium">{SelectedSlide.Duration}s</span>
                    </div>
                  </div>
                  ...
                  <div className="absolute top-6 left-6 max-w-[70%] px-4 py-2 bg-black/60 backdrop-blur-md rounded-lg border border-gray-700/50 z-20 group">
                    {IsEditingName ? (
                      <div className="flex items-center">
                        ...
                        />
                        <span className="text-gray-400 text-xs ml-1">{GetFileExtension(SelectedSlide.Name)}</span>
                      </div>
                    ) : (
                      <div 
                ...
                <div className="flex flex-col items-center justify-center h-full text-center p-6">
                  ...
                  <p className="text-gray-400 text-lg max-w-md">Wybierz lub dodaj slajd, aby zobaczyć podgląd</p>
                </div>
              )}
            </div>
            
            <div className={`h-24 mt-auto rounded-xl p-3 hidden md:flex flex-wrap gap-2 border ${IsDragging ? 'border-rose-500 bg-rose-500/10' : 'border-gray-700/50 bg-gray-800/20'} transition-all duration-300`}
                ...
                {_Slides.length > 0 && (
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs text-white bg-rose-500/20 px-3 py-1 rounded-full border border-rose-500/30">
                      {_Slides.length} {_Slides.length === 1 ? 'slajd' : _Slides.length < 5 ? 'slajdy' : 'slajdów'}
                    </span>
                  </div>
                )}
              </div>
              ...
              >
                {_Slides.length > 0 ? (
                    ...
                    <div className="space-y-2.5">
                      {_Slides.map((Slide, Index) => (
                        <motion.div
                          ...
                          className="relative"
                        >
                          {DragOverSlide === Slide.Id && DragPosition === 'above' && (
                          ...
                          <div className={`group rounded-lg overflow-hidden transition-all flex border 
                            ...
                            onDrop={(e) => { e.preventDefault(); HandleDropOnSlide(); }}
                          ...
                          >
                            <div className="h-20 w-30 overflow-hidden flex-shrink-0 relative">
                              ...
                              />
                              <div className={`absolute inset-0 ${SelectedSlide?.Id === Slide.Id ? 'bg-rose-500/10' : 'bg-black/30 group-hover:bg-black/10'} transition-colors duration-200`}></div>
                              {(DeletingSlideId === Slide.Id || DeletingSlideId === "all") && (
                            ...
                            <div className="flex-1 p-2 flex flex-col justify-between">
                              ...
                              <div className="flex items-center justify-between mt-1 select-none">
                                <div className="flex items-center">
                                  <label htmlFor={`duration-${Slide.Id}`} className="text-gray-400 text-xs mr-2">Czas:</label>
                                  <div className="relative flex items-center">
                                    <input 
                                      ...
                                      min={1}
                                      max={60}
                                      onChange={(e) => HandleDurationChange(Slide.Id, parseInt(e.target.value) || 1)}
                                ...
                                <div className="flex space-x-1">
                                  <button 
                                    onClick={(e) => { e.stopPropagation(); HandleDownload(Slide.ImageData, Slide.Name); }}
                          ...
                          {DragOverSlide === Slide.Id && DragPosition === 'below' && Index === _Slides.length - 1 && (
                  ...
                  <div className="flex flex-col items-center justify-center h-full py-8">
                    <div 
                      className="rounded-full bg-gray-800/50 p-6 mb-4 relative overflow-hidden"
                    >
                    ...
                    <p className="text-gray-400 text-sm max-w-xs text-center">Brak slajdów. Dodaj slajdy, aby rozpocząć.</p>
