...
type VoteTab = "submit" | "recent";

interface NavbarProps {
  logoHref?: string;
  activeTab?: VoteTab;
  onChangeTab?: (Tab: VoteTab) => void;
}

const Navbar = ({ logoHref, activeTab, onChangeTab }: NavbarProps) => {
  ...
  const Logo = () => (
    <>
      <Image
        ...
        priority
      />
      <div className="absolute -inset-[3px] border border-white/20 rounded-full animate-ping opacity-20"></div>
    </>
  );
  ...
  return (
    <>
      <div className="fixed top-0 left-0 w-full z-50">
        <div className="md:hidden bg-black/80 backdrop-blur-sm border-b border-white/10 px-4 py-2.5">
        ...
        <header className="w-full px-4 md:px-10 py-4 md:py-6 bg-black/90 backdrop-blur-sm border-b border-white/10">
          <div className="max-w-6xl mx-auto flex justify-between items-center">
            ...
            <div className="flex items-center gap-2 md:gap-4 flex-wrap md:flex-nowrap justify-end">
                ...
                <div className="flex md:hidden items-center mr-2">
                  ...
                  >
                    <button
                      ...
                      type="button"
                      className={`relative px-3 py-1.5 text-xs font-semibold uppercase tracking-[0.18em] transition-colors rounded-full ${
                        activeTab === "submit"
                          ? "text-white border border-white"
                          : "text-white/60 hover:text-white/90 border border-transparent"
                      } ${_SpaceGrotesk.className}`}
                      aria-label="Przełącz na wysyłanie piosenek"
                    >
                      Wyślij
                    </button>
                    <button
                      ...
                      aria-label="Przełącz na tryb głosowania"
                    >
                      Głosowanie
                    </button>
                  </nav>
              ...
              <div
                className={`hidden md:flex font-mono text-sm text-white/70 ${_JetBrainsMono.className} tracking-wider bg-white/5 backdrop-blur-sm py-2 px-4 border border-white/10 rounded-full`}
              >
