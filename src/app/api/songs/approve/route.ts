...
export async function POST(Request: NextRequest) {
  try {
    ...
    if (!Song) {
      return NextResponse.json(
        { error: "Piosenka nie została znaleziona w propozycjach" },
        { status: 404 },
      );
    }
    ...
    const ApprovedSong = await Prisma.approvedSong.create({
      data: {
        ...
        Uri: Song.Uri,
        Upvotes: Song.Votes.filter((v: { IsUpvote: boolean }) => v.IsUpvote)
          .length,
        Downvotes: Song.Votes.filter((v: { IsUpvote: boolean }) => !v.IsUpvote)
          .length,
      },
    });
    ...
    if (Song.Votes.length > 0) {
      await Promise.all(
        Song.Votes.map((Vote: { Fingerprint: string; IsUpvote: boolean }) =>
          Prisma.vote.create({
            data: {
